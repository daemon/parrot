...
class SentimentClassification(Task):
    ...
    async def on_query(self, evaluator, llm, query: str, final_results: 'EvaluationResults', running_results: List[Dict[str, Any]]):
        response = await llm.chat_complete([{
            'role': 'user',
            'content': f'Grade the sentiment of the following sentence as one of negative, neutral, or positive: "{query}". Do not explain.'
        }])

        grade = await evaluator.chat_complete([{
            'role': 'user',
            'content': f'Given the sentence "{query}", is the sentiment polarity "{response}"? Respond with only "yes" or "no"; do not explain.'}
        ])
...
class DocumentClassification(Task):
    ...
    async def on_query(self, evaluator, llm, query: str, final_results: 'EvaluationResults',
                       running_results: List[Dict[str, Any]]):
        response = await llm.chat_complete([{
            'role': 'user',
            'content': f'What is the topic of the following passage: "{query}"? Do not explain.'
        }])

        grade = await evaluator.chat_complete([{
            'role': 'user',
            'content': f'Given the passage "{query}", is the topic of the passage "{response}"? Respond with only "yes" or "no"; do not explain.'}
        ])
